...
(function () {
  ...
  function renderAuthorBar(post) {
    ...
    if (nameEl)   nameEl.textContent   = post.author || 'Digital World Team';
    if (metaEl)   metaEl.innerHTML     =
      formatDate(post.date) +
      '<span class="dot"></span>' +
      (post.readTime || post.readingTime || '5 min read') +
      '<span class="dot"></span>' +
      (post.category || 'Digital Blogging');
  }
  ...
  function renderHero(post) {
    ...
    if (heroEl) {
      heroEl.src = toAbsolutePath(post.coverImage) || 'https://source.unsplash.com/1200x500/?digital,blogging';
    ...
    var cards = picks.map(function (p) {
      ...
      return '<a href="/blog/' + p.slug + '" class="post-card">' +
        ...
        '<div class="post-card-body">' +
          '<span class="badge">' + (p.category || 'Digital Blogging') + '</span>' +
  ...
  function init() {
    var slug = getSlug();
    if (!slug) {
      document.title = 'Post Not Found -- Digital World';
